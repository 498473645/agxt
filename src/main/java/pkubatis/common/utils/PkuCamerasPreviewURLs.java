...
public class PkuCamerasPreviewURLs {
    ...
    public static String getPlayback(String url,String secret,String deviceId, String channelId,String startTime,String endTime){
        try {
            ...
            //ssrc = intToHex(Integer.parseInt(ssrc));
            return ssrc;
        }catch (Exception e){
            return null;
        }
	...
	* @return
	*/
	public static String getdevice(String url,String secret,String deviceId){
	 try {
	 	RestTemplate restTemplate = new RestTemplate();
	 	url=url+"/api/devices/"+deviceId+"?secret="+secret;
	 	String result = restTemplate.getForObject(url, String.class);
	     JSONObject object = JSON.parseObject(result);
	     String channelMap = object.get("channelMap").toString();
	     return channelMap;
	 }catch (Exception e){
	     return null;
	 }
	...
	*/
	public static String addFFmpegSource(String url,String secret,String src_url, String dst_url){
	 ...
	 /**
     * 通过fork FFmpeg进程的方式拉流代理，支持任意协议 回放模式
	* @return
	*/
	public static String playbackAddFFmpegSource(String url,String secret,String src_url, String dst_url){
	 try {
	 	RestTemplate restTemplate = new RestTemplate();
	 	url=url+"/index/api/addFFmpegSource?src_url="+src_url+"&dst_url="+dst_url+"&secret="+secret+"&timeout_ms=10000&ffmpeg_cmd_key=4";
	 	String result = restTemplate.getForObject(url, String.class);
	     JSONObject object = JSON.parseObject(result);
	     JSONObject data = JSON.parseObject(object.get("data").toString());
	     String key = data.get("key").toString();
	...
	*/
	...
	public static String addStreamProxy(String url,String secret,String vhost,String app,String stream ,String rtmpUrl) {
		RestTemplate restTemplate = new RestTemplate();
		url=url+"/index/api/addStreamProxy?secret="+secret+"&vhost="+vhost+"&app="+app+"&stream="+stream+"&url="+rtmpUrl+"&rtp_type=0";
		Map<String, String> mapValue = new HashMap<String, String>();
		HttpEntity<Map<String, String>> request = new HttpEntity<Map<String, String>>(mapValue);
		String result = restTemplate.postForObject(url, request,String.class);
		JSONObject object = JSON.parseObject(result);
